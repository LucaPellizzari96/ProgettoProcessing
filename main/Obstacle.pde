class Obstacle {
  ...
  protected float x = 500.0 * cos(radians(angle)); // 500 perche sarebbe 250.0 *...* 2 con 2 = a (dimensione) dell'ellisse su x
  protected float y = 250.0 * sin(radians(angle));
  private float dim = 30.0;
  private float frame = 0; // conto i frame per gestire il colore dell'ostacolo
